"""
Modified from: https://github.com/mr-eggplant/EATA/blob/main/eata.py
...
class EATA(nn.Module):
    ...
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, episodic=False,
                 e_margin=math.log(1000) / 2 - 1, d_margin=0.05, mask=None):
        super().__init__()
        self.model = model
...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, mask, fisher_alpha=50.0,
    ...
    if current_model_probs is not None:
        cosine_similarities = F.cosine_similarity(current_model_probs.unsqueeze(dim=0).cpu(),
                                                  outputs[filter_ids_1].cpu().softmax(1), dim=1)
        filter_ids_2 = torch.where(torch.abs(cosine_similarities) < d_margin)
        entropys = entropys[filter_ids_2]
        ids2 = filter_ids_2
        updated_probs = update_model_probs(current_model_probs.cpu(),
                                           outputs[filter_ids_1][filter_ids_2].cpu().softmax(1))
    else:
        updated_probs = update_model_probs(current_model_probs, outputs[filter_ids_1].cpu().softmax(1))
